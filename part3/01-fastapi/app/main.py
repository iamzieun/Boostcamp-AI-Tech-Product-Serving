...
from model import MyEfficientNet, get_model
...
orders = []
# 실무에서는 보통 이 경우에 데이터베이스를 이용하여 주문을 저장하지만, 아직 데이터베이스를 학습하지 않았으므로 in-memory인 리스트에 저장
...
def get_orders(order_id: UUID):
    ...
    # 만약 get_order_by_id에서 아무런 데이터가 없다면? 빈 리스트가 나온다면?
    if not order:
...
def get_order_by_id(order_id: UUID):
    ...
    # generator: 메모리를 더 절약하여 사용할 수 있음
    # iter, next 키워드로 검색
    # 이터레이터, 이터러블, 제너레이터 관련해서는 파이썬 면접에서 많이 나오는 소재. GIL
...
async def make_order(files: List[UploadFile] = File(...),
                     ...
                     config: Dict[str, Any] = Depends(get_configs)):
    # Depends: 의존성 주입
...
# TODO: Order, Product Class 구현
    # TODO: Order의 products 필드로 Product의 List (하나의 주문에 여러 개의 제품이 있을 수 있음)
